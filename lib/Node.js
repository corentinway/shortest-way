...
/**
 * Add two edges between nodes (a -> b & b -> a). The purpose of this function 
 * is to simplify code dealing with indirected graphs.
 * @param node {Node} node to add
 * @param node {Number} weight of the edge.
 */
Node.prototype.connectWith = function ( node, weight ) {
	node.addNext( this, weight );
	this.addNext( node, weight );
}
